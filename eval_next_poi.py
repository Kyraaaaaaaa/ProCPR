...
def parse_config():
    ...
    parser.add_argument('--model_path', type=str, default='Llama-2-7b-longlora-32k-ft', help='your model path')
    parser.add_argument('--data_path', type=str, default="./test.bin", help='')
    parser.add_argument('--output_dir', type=str, default="UPTDNet_dataset/Gowalla/Dallas_Austin/models/nogeo_models/checkpoint-125535", help='')
    parser.add_argument("--input_path", type=str, default="UPTDNet_dataset")
    parser.add_argument("--dataset", type=str, default="Gowalla", choices=['Foursquare','Gowalla'])
    parser.add_argument("--home_city", type=str, default="Dallas", choices=['Washington','Dallas','LosAngeles'])
    parser.add_argument("--current_city", type=str, default="Austin", choices=['Baltimore','Austin','SanFrancisco'])
    parser.add_argument('--test_file', type=str, default="test_nogeo.json",
                        help='')
...
def main(args):
    ...
    model.eval()
    if output_dir:
        trainable_params = os.path.join(output_dir, "trainable_args.bin")
    ...
    lines = jload(dataset_path / f"{args.home_city}_{args.current_city}" / f"{args.test_file}")
    # with open(data_path + f"{args.test_file}", "r") as file:
    #     lines = file.readlines()
    # lines = [{"question": q, "answer": a} for line in test_data for (q, a) in [line['question'].split("<answer>:")]]
    lines = list(filter(lambda d: "<question>:" in d["question"], lines))
    ...
    for index, line in tqdm(enumerate(lines), desc="Processing lines", total=len(lines)):
        prompt1, gt = line['question'], line['answer']
        # tmp1, tmp2 = prompt1.split(', which POI ID user ')
        tmp1, tmp2 = prompt1.split(', Which POI id will user ')
        # tmp1, tmp2 = prompt1.split(', predict POI at ')
        time = tmp1[-19:]
        # time = tmp2[:19]
        user_id = tmp2.split(' will')[0]
        # user_id = tmp1.split('<question>: User: ')[1][0]
        prompt = prompt1 + '\n <answer>: ' + f'At {time}, user {user_id} will visit POI id '

        # prompt = prompt1.replace('<question>:', '<question>:') + '<answer>: ' + f'At {time}, user {user_id} will visit POI id '
        ...
        while i < 1:
            try:
                ...
                prediction = tokenizer.decode(generated_ids[i], skip_special_tokens=True)
    ...
    print(f"Prediction List: {predict_list}")
