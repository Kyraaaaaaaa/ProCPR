...
save_path = f'dataset/{dataset}/{city_A}_{city_B}'
if dataset == "Gowalla":
    if city_A in ['Dallas', 'Austin']:
...
data.to_csv("Gowalla_LosAngeles_SanFrancisco.csv")

home_city_records = data[data['cross_city_mode'] == f'{city_A}_{city_A}']
transition_records = data[data['cross_city_mode'] == f'{city_A}_{city_B}']
travel_city_records = data[data['cross_city_mode'] == f'{city_B}_{city_B}']
...
def split_train_test_by_mode(df, train_ratio=0.8):
    ...
    for traj, group in df.groupby('pseudo_session_trajectory_id'):
        ...
        transition_group = group[group['cross_city_mode'] == f'{city_A}_{city_B}']
        travel_group = group[group['cross_city_mode'].isin([f'{city_A}_{city_A}', f'{city_B}_{city_B}'])]
