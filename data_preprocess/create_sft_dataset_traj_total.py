...
def parse_args():
    ...
    parser.add_argument("--city_A", type=str, default="Austin", choices=['Dallas', 'LosAngeles'], help="Home city")
    parser.add_argument("--city_B", type=str, default="Dallas", choices=['Austin', 'SanFrancisco'], help="Current city")
...
def main(args):
    ...
    profiles_path = dataset_path / f"{args.city_A}_{args.city_B}" 

    with open(dataset_path / f"{args.city_A}_{args.city_B}" / "train_qa_pairs.json") as f:
        raw_data_train = json.load(f)

    with open(dataset_path / f"{args.city_A}_{args.city_B}" / "test_qa_pairs.txt") as f:
    ...
    with open(dataset_path / f"{args.city_A}_{args.city_B}" / "train.json", "w") as f:
        json.dump(train_data, f, indent=4)

    with open(dataset_path / f"{args.city_A}_{args.city_B}" / "test.json", "w") as f:
