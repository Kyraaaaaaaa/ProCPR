...
def generate_qa_pairs(main_data, if_test=False, args=None):
    ...
    for user in tqdm(main_data['userid'].unique()):
        ...
        for traj_id in user_data['pseudo_session_trajectory_id'].unique():
            ...
            value = {'Dallas': 11374, 'Austin': 4721, 'LosAngeles': 3207, 'SanFrancisco': 3682}[args.city_B]
...
def main():
    ...
    parser.add_argument("--city_A", type=str, default="LosAngeles", choices=['Dallas','LosAngeles'])
    parser.add_argument("--city_B", type=str, default="SanFrancisco", choices=['Austin','SanFrancisco'])

    # Parse the arguments
    args = parser.parse_args()

    # Your processing code here
    print(f"Processing dataset: {args.dataset}_{args.city_A}_{args.city_B}")
    path = f"{args.input_path}/{args.dataset}/{args.city_A}_{args.city_B}"
    # Read the data
